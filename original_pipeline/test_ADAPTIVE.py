...
class DataTests(unittest.TestCase):
    ...
    def build(self,**kw):
        with contextlib.redirect_stdout(io.StringIO()):return D.prepare(self.data,self.base/'prepared',**kw)
    ...
    def test_train_test_carve_valid_only(self):
        self.add('Train',(15,20));self.add('Test',(4,6));m=self.build();self.assertEqual(m['generated_splits'],['valid']);self.assertEqual(D.Split(m,'test').n,10);self.assertEqual(len(m['rows']),45)
    def test_flat(self):
        self.add('',(15,20));m=self.build();self.assertEqual(m['generated_splits'],['test','valid']);self.assertEqual(sum(D.Split(m,s).n for s in ('train','valid','test')),35)
    def test_keep_valid_when_test_missing(self):
        self.add('train',(12,12));self.add('val',(3,4));m=self.build();self.assertEqual(m['generated_splits'],['test']);self.assertEqual(D.Split(m,'valid').n,7)
    def test_identical_content_not_removed(self):
        for s in ('train','valid','test'):self.add(s,(3,3))
        m=self.build();self.assertEqual(len(m['rows']),18);self.assertEqual(len({r['sha256'] for r in m['rows']}),1)
    def csv(self,rows):
        p=self.base/'map.csv'
        ...
        for cls in ('a','b'):
            for group in range(10):
                for image in range(2):
                    p=self.img(Path('arbitrary')/(f'{cls}_{group}_{image}.png'));rows.append(dict(path=str(p),label=cls,split='train',group=f'{cls}_{group}'))
        m=self.build(csv_path=self.csv(rows));self.assertEqual(m['group_unit'],'supplied_group');groups={s:set(D.Split(m,s).groups) for s in ('train','valid','test')};self.assertFalse(groups['train']&groups['test']);self.assertFalse(groups['train']&groups['valid'])
    ...
    def test_content_mutation_detected(self):
        for s in ('train','valid','test'):self.add(s,(3,3))
        m=self.build();D.check_content(m,('train','valid'));r=next(r for r in m['rows'] if r['split']=='train');Path(r['path']).write_bytes(b'changed')
    ...
    def test_paired(self):
        r=R.paired([0,1],[0,1],[0,1],['p1','p2'],100);self.assertEqual(r['delta_accuracy'],0);self.assertEqual(r['p_raw'],1)
