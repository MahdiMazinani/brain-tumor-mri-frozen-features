...
def install(V,run,final=False):
    ...
    V.load_bed=load_bed
    # Do not attach old, hardcoded cross-corpus measurements to a new dataset.
    V.S.cohort_overlap=lambda key:{'corpus_shared_with':'','cohort_test_in_other_dev':'',
                                  'cohort_overlap_note':'not audited for this dataset'}
    class Store:
        def __init__(self,cfg,bed):
            self.cfg=cfg;self.bed=bed;self.binds=[];self.memory={}
            self.cache=Path(run)/'native_cache';self.cache.mkdir(parents=True,exist_ok=True)
        def backbone_features(self,role,backbone,tta_n):
            ...
            token=hashlib.sha256((m.digest()+backbone+PRE.preprocess_rule_id(self.cfg.img_size)).encode()).hexdigest()
            xp=self.cache/(backbone+'_'+role+'_'+token+'.npy')
            yp=self.cache/(backbone+'_'+role+'_'+token+'_labels.npy')
            if xp.exists() and yp.exists():
                ...
                net,dim=V.build_extractor_v5(backbone,device,self.cfg.img_size)
                net.eval(); transform=PRE.build_transform(self.cfg.img_size,backbone);chunks=[]
                with torch.inference_mode():
                    for i in range(0,m.n,16):
                        ...
                        z=net(x)
                        if hasattr(z,'logits'):z=z.logits
                        z=z.flatten(1)
                ...
                X=np.concatenate(chunks).astype(np.float32);y=m.y()
                np.save(xp,X);np.save(yp,y)
                del net
                if device.type=='cuda':torch.cuda.empty_cache()
            if X.shape!=(m.n,V.ALL_DIM[backbone]) or not np.array_equal(y,m.y()):
                raise RuntimeError('Native cache does not match manifest.')
            if not np.isfinite(X).all():raise RuntimeError('Nonfinite embeddings.')
            self.binds.append({'role':role,'backbone':backbone,'tta_requested':1,'tta_effective':1,'file':str(xp)})
            self.memory[key]=(X,y);return X,y
        ...
        def effective_tta(self,role,spec=None,requested=None):
            return {'requested':[1],'effective':[1],'substituted':[],'uniform':True,'notes':[]}
