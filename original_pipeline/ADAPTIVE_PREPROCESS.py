...
def build_transform(img_size=224,backbone='resnet18'):
    ...
    return T.Compose([T.Resize((img_size,img_size),interpolation=InterpolationMode.BILINEAR,antialias=True),T.ToTensor(),T.Normalize(MEAN,STD)])

def provenance(img_size=224):
    return {'img_size':img_size,'resize':'square, bilinear, antialias=True','channels':'PIL convert RGB','mean':list(MEAN),'std':list(STD),'tta':['identity'],'channel_audit':'not performed on this dataset'}
