...
def folders(root,layout='auto'):
    ...
    if splitdirs and len(splitdirs)!=len(children):
        raise DataError('Mixed split folders and other directories; choose --layout flat or use --csv explicitly.')
    for r,base in sources.items():
        classdirs=sorted(p for p in base.iterdir() if p.is_dir() and not p.name.startswith('.'))
        for cls in classdirs:
            for p in sorted(cls.rglob('*')):
                if p.is_file() and p.suffix.lower() in EXT:
                    rows.append({'path':str(p.resolve()),'label':cls.name,'split':r,'group':None})
                elif p.is_file() and (p.suffix.lower() in ('.dcm','.nii','.mat','.npy') or p.name.lower().endswith('.nii.gz')):
                    raise DataError('Unsupported medical/array format: '+str(p)+'; convert explicitly before using this image classifier.')
...
def csv_rows(root,path):
    with open(path,encoding='utf-8-sig',newline='') as f:
        ...
        for n,r in enumerate(reader,2):
            ...
            rows.append({'path':str(p.resolve()),'label':r['label'].strip(),
                         'split':role(r.get('split','')),'group':r.get('group','').strip() or None})
    return rows
...
def validate(rows):
    ...
    for r in rows:
        ...
        seen.add(r['path'])
        key=r['group']
        prior=groups.setdefault(key,(r['label'],r['split']))
        if prior!=(r['label'],r['split']):
            raise DataError('A group crosses splits or labels: '+str(key)+'. Original assignments were NOT changed.')
...
def carve(rows,target,fraction,seed):
    ...
    for label,ids in sorted(pools.items()):
        ...
        n=max(1,min(len(ids)-1,round(len(ids)*fraction)))
        moved.update(ids[:n])
    for r in rows:
...
def prepare(data,out,csv_path=None,layout='auto',val_fraction=.2,test_fraction=.2,seed=42,normal_class=None):
    ...
    rows=csv_rows(root,csv_path) if csv_path else folders(root,layout)
    with_group=[bool(r['group']) for r in rows]
    ...
    validate(rows)
    original={r['path']:r['split'] for r in rows}
    present={r['split'] for r in rows}; generated=[]
    if 'train' not in present:raise DataError('A training pool is required.')
    if 'test' not in present:
        carve(rows,'test',test_fraction,seed);generated.append('test')
    if 'valid' not in present:
        fraction=val_fraction/(1-test_fraction) if 'test' in generated else val_fraction
        carve(rows,'valid',fraction,seed+1);generated.append('valid')
    validate(rows)
    ...
    # Content hashes record file identity only. No hashes are compared across files;
    # no duplicate detection, removal, balancing or source-file mutation occurs.
    from PIL import Image
    modes=Counter()
    for r in rows:
        with Image.open(r['path']) as image:
            if image.mode in ('I','F') or image.mode.startswith('I;16'):
                raise DataError('High-bit-depth image needs an explicit intensity conversion: '+r['path'])
            if getattr(image,'n_frames',1)!=1:
                raise DataError('Multi-frame image must be converted to explicit slices: '+r['path'])
            modes[image.mode]+=1
            image.verify()
        r['sha256']=sha(r['path'])
    rows.sort(key=lambda r:(r['split'],r['label'],r['path']))
    manifest={'version':1,'root':str(root),'classes':classes,'normal_class':normal_class,
              'group_unit':'supplied_group' if grouped else 'image','seed':seed,
              'generated_splits':generated,'rows':rows,
              'warning':'User-supplied groups must identify independent units. With image IDs, patient overlap is unassessed. No deduplication was performed.'}
    save(out/'dataset.json',manifest)
    summary={'images':len(rows),'classes':classes,'group_unit':manifest['group_unit'],
             'counts':{s:dict(Counter(r['label'] for r in rows if r['split']==s)) for s in ('train','valid','test')},
             'group_counts':{s:len({r['group'] for r in rows if r['split']==s}) for s in ('train','valid','test')},
             'generated_splits':generated,'seed':seed,'image_modes':dict(modes),'deduplication_performed':False,
             'class_balancing_resampling_performed':False,'warning':manifest['warning']}
    save(out/'dataset_summary.json',summary)
...
def check_content(manifest,roles):
    for r in manifest['rows']:
        if r['split'] in roles and (not Path(r['path']).is_file() or sha(r['path'])!=r['sha256']):
            raise DataError('Image content changed after manifest preparation: '+r['path'])

class Split:
    def __init__(self,manifest,split):
        self.role=split;self.rows=[r for r in manifest['rows'] if r['split']==split]
        self.classes=manifest['classes']; self.files=[r['path'] for r in self.rows]
        self.groups=[r['group'] for r in self.rows];self.n=len(self.rows)
        self.recipe={'source':'dynamic manifest','role':split,'generated':split in manifest['generated_splits']}
    def y(self):
        import numpy as np
